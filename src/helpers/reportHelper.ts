...
export function getCheckGoalsModified(originalGoals: ICheckGoal[], newGoals: ICheckGoal[]) {
    ...
    for (const originalGoal of originalGoals) {
        ...
        const goalFinded = newGoals.find((newGoal) => newGoal.id === originalGoal.id);

        if (!goalFinded) {
            deleted.push(originalGoal);
        } else if (JSON.stringify(goalFinded) !== JSON.stringify(originalGoal)) {
...
export function getProgressGoalsModified(originalGoals: IProgressGoal[], newGoals: IProgressGoal[]) {
