export function getTimeElapsed(dataAtualizacao: string): string {
    const now = new Date();
...
export function getCreatedTimeElapsed(createdDate: string): string {
    return `Created ${getTimeElapsed(createdDate)} ago`;
}

export function getUpdatedTimeElapsed(updatedDate: string): string {
    return `Updated ${getTimeElapsed(updatedDate)} ago`;
}
